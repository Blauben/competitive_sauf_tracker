...
class DrinkSelectorBody extends StatefulWidget {
  ...
  const DrinkSelectorBody({Key? key, required this.drinkCategory}) : super(key: key);
...
class _DrinkSelectorBodyState extends State<DrinkSelectorBody> {
  ...
  Widget build(BuildContext context) {
    return _DrinkSelectorListView(drinkCategory: widget.drinkCategory, drinkListCubit: DrinkListCubit(),);
  }
}
...
class _DrinkSelectorListView extends StatelessWidget {
  ...
  const _DrinkSelectorListView({Key? key, required this.drinkCategory, required this.drinkListCubit}) : super(key: key);
        ...
        if(state.getDrinkByCategory(drinkCategory.index+1).isEmpty) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              return DrinkSelectorItem(drink: state.getDrinkByCategory(drinkCategory.index+1)[index]);
            },
          );
        }
